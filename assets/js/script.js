        ...
        .then(function (coords) {
            ...
            fetch(requestUrl)
                ...
                .then(function (data) {
                    ...
                    document.querySelector('.current h4').textContent = data.city.name + ' ' + dayjs(data.list[0].dt_txt).format('M/DD/YYYY');
                    document.querySelector('#icon').setAttribute('src', 'https://openweathermap.org/img/wn/' + data.list[0].weather[0].icon + '@2x.png');
                    document.querySelector('#icon').setAttribute('alt', data.list[0].weather[0].description);
                    document.querySelector('.current .l1').textContent = 'Temp: ' + ((data.list[0].main.temp - 273.15) * 9 / 5 + 32).toFixed(1) + '°F';
                    document.querySelector('.current .l2').textContent = 'Wind: ' + (data.list[0].wind.speed).toFixed(1) + ' ' + 'MPH';
                    document.querySelector('.current .l3').textContent = 'Humidity: ' + data.list[0].main.humidity + '%';
